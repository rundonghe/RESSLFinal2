...
class Curriculum_Labeling(InductiveEstimator,DeepModelMixin,ClassifierMixin):
    def __init__(self,
    			 alpha=0.75,
                 lambda_u=75,
                 T=0.5,
    ...
    def train(self,lb_X,lb_y,ulb_X,lb_idx=None,ulb_idx=None,*args,**kwargs):
        ...
        targets_x = torch.zeros(batch_size, self.num_classes).scatter_(1, lb_y.view(-1,1), 1)
        with torch.no_grad():
    ...
    def get_loss(self,train_result,*args,**kwargs):
        lb_logits, lb_y, w_ulb_logits, s_ulb_logits = train_result
   		Lx, Lu, w=nn.CrossEntropyLoss()(lb_logits, lb_y, w_ulb_logits, s_ulb_logits,self._epoch+self.it_epoch/self.num_it_epoch)
        loss=Lx+w*Lu
        return loss
